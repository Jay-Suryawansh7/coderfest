    ...
    >
      ...
      <div className="aspect-[4/3] bg-gradient-to-br from-primary/10 to-accent/10 relative overflow-hidden">
        ...
        {monument.unescoStatus && (
          <div className="absolute top-3 left-3 px-2 py-1 bg-accent text-text text-xs font-semibold rounded-full flex items-center gap-1">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-border text-xs text-text-light">
          {monument.yearBuilt && (
            <span>Built: {monument.yearBuilt}</span>
          )}
          {monument.style && (
            <span className="truncate">{monument.style}</span>
          )}
        </div>
      </div>
    </Link>
